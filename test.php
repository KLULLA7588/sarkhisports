...
if(isset($_GET['action'])&&$_GET['action']==='generate_pdf'){
    ...
    class BillPDF extends TCPDF{
        public function Header(){$this->SetFillColor(22,38,58);$this->Rect(0,0,210,30,'F');$this->SetFont('helvetica','B',18);$this->SetTextColor(0,170,255);$this->SetXY(10,6);$this->Cell(130,8,'Sarthi Sports Wear',0,0,'L');$this->SetFont('helvetica','B',9);$this->SetTextColor(255,255,255);$this->SetFillColor(0,74,136);$this->Rect(148,5,54,10,'F');$this->SetXY(148,7);$this->Cell(54,6,$GLOBALS['pdf_invoice_no'],0,0,'C');$this->SetFont('helvetica','',8);$this->SetTextColor(180,210,240);$this->SetXY(10,17);$this->Cell(130,5,'Ph: 0762-0425141  |  Mob: 9422107750  |  Nagpur, Maharashtra',0,0,'L');$this->SetDrawColor(0,170,255);$this->SetLineWidth(0.8);$this->Line(0,30,210,30);$this->SetLineWidth(0.2);}
        public function Footer(){$this->SetY(-14);$this->SetFont('helvetica','I',7);$this->SetTextColor(130,150,170);$this->Cell(0,6,'Thank you for your order!  *  Sarthi Sports Wear  *  Ph: 0762-0425141 | 9422107750',0,0,'C');$this->Ln(5);$this->Cell(0,4,'Page '.$this->getAliasNumPage().' of '.$this->getAliasNbPages(),0,0,'C');}
    }
    $pdf=new BillPDF('P','mm','A4',true,'UTF-8',false);
    $pdf->SetCreator('Sarthi Sports Wear');$pdf->SetAuthor('Sarthi Sports Wear');$pdf->SetTitle('Bill '.$invoice_no);$pdf->SetMargins(10,34,10);$pdf->SetHeaderMargin(0);$pdf->SetFooterMargin(16);$pdf->SetAutoPageBreak(true,20);$pdf->AddPage();
    $pdf->SetFillColor(22,38,58);$pdf->SetDrawColor(46,64,96);$pdf->SetLineWidth(0.3);$pdf->Rect(10,34,190,26,'FD');$pdf->SetFont('helvetica','B',8);$pdf->SetTextColor(90,122,154);$pdf->SetXY(14,37);$pdf->Cell(40,4,'CUSTOMER NAME',0,1);$pdf->SetFont('helvetica','B',12);$pdf->SetTextColor(208,228,247);$pdf->SetXY(14,41);$pdf->Cell(80,6,strtoupper($customer_name),0,0);
    $info=array(array('ORDER DATE',fmt_date($order_date)),array('DELIVERY DATE',fmt_date($delivery_date)));
    if($advance_date)$info[]=array('ADVANCE DATE',fmt_date($advance_date));
    $info[]=array('TYPE',$bill_type);$info[]=array('STATUS',$bill_status);
    foreach($info as $i=>$info_item){$label=$info_item[0];$val=$info_item[1];$col=($i%2===0)?105:155;$row=36+intdiv($i,2)*10;$pdf->SetFont('helvetica','B',7);$pdf->SetTextColor(90,122,154);$pdf->SetXY($col,$row);$pdf->Cell(46,4,$label,0,0);$pdf->SetFont('helvetica','',8);$pdf->SetTextColor(208,228,247);$pdf->SetXY($col,$row+4);$pdf->Cell(46,4,$val,0,0);}
    $pdf->SetY(63);$col_w=array(8,28,20,22,20,25,12,16,22,18);$headers=array('#','Product','Pattern','Fabric','Color','Sizes & Qty','MRP','Net Rate','Amount','Print Chg');$aligns=array('C','L','L','L','L','L','R','R','R','R');
    $pdf->SetFillColor(15,25,45);$pdf->SetDrawColor(46,64,96);$pdf->SetTextColor(90,122,154);$pdf->SetFont('helvetica','B',7);$pdf->SetLineWidth(0.25);
    foreach($headers as $hi=>$h){$pdf->Cell($col_w[$hi],6,$h,1,0,$aligns[$hi],true);}$pdf->Ln();
    $row_n=0;$grand_qty=0;$grand_print=0;
    foreach($items as $item){$product=ucwords(str_replace('_',' ',isset($item['product'])?$item['product']:''));$pattern=isset($item['pattern'])?$item['pattern']:'';$fabric=isset($item['fabric'])?$item['fabric']:'';$color=isset($item['color'])?$item['color']:'';$sizes=isset($item['sizes'])?$item['sizes']:array();$print_chg=floatval(isset($item['print_charges'])?$item['print_charges']:0);$size_lines=array();$item_qty=0;$item_amt=0;$first_mrp=0;$first_nr=0;
    foreach($sizes as $sz){$qty=floatval(isset($sz['qty'])?$sz['qty']:0);$nr=floatval(isset($sz['netrate'])?$sz['netrate']:0);$amt=floatval(isset($sz['amount'])?$sz['amount']:0);$mrp=floatval(isset($sz['mrp'])?$sz['mrp']:0);if($qty>0){$size_lines[]=(isset($sz['size'])?$sz['size']:'').' x '.intval($qty);$item_qty+=$qty;$item_amt+=$amt;if(!$first_mrp){$first_mrp=$mrp;$first_nr=$nr;}}}
    $grand_qty+=$item_qty;$grand_print+=$print_chg;$row_total=$item_amt+$print_chg;$sizes_text=implode("\n",$size_lines);
    if($row_n%2===0){$pdf->SetFillColor(26,37,53);}else{$pdf->SetFillColor(30,45,64);}$pdf->SetTextColor(208,228,247);$pdf->SetDrawColor(46,64,96);$pdf->SetFont('helvetica','',7);
    $lines=max(1,count($size_lines));$row_h=max(6,$lines*5+2);$y0=$pdf->GetY();
    if($y0+$row_h>260){$pdf->AddPage();$pdf->SetFillColor(15,25,45);$pdf->SetFont('helvetica','B',7);$pdf->SetTextColor(90,122,154);foreach($headers as $hi=>$h){$pdf->Cell($col_w[$hi],6,$h,1,0,$aligns[$hi],true);}$pdf->Ln();$y0=$pdf->GetY();}
    $cells=array($row_n+1,$product,$pattern,$fabric,$color,$sizes_text,$first_mrp?number_format($first_mrp,2):'--',$first_nr?number_format($first_nr,2):'--',number_format($row_total,2),$print_chg>0?number_format($print_chg,2):'--');
    $x0=10;foreach($cells as $ci=>$cell_val){$pdf->SetXY($x0+array_sum(array_slice($col_w,0,$ci)),$y0);if($ci===5){$pdf->MultiCell($col_w[$ci],$row_h,$cell_val,1,'L',true,0);}else{$pdf->Cell($col_w[$ci],$row_h,$cell_val,1,0,$aligns[$ci],true);}}$pdf->Ln($row_h);$row_n++;}
    $pdf->Ln(4);$y_totals=$pdf->GetY();
    $pdf->SetFillColor(22,38,58);$pdf->SetDrawColor(46,64,96);$pdf->Rect(10,$y_totals,88,40,'FD');$pdf->SetFont('helvetica','B',8);$pdf->SetTextColor(90,122,154);$pdf->SetXY(14,$y_totals+3);$pdf->Cell(40,5,'TOTAL QTY',0,0);$pdf->SetFont('helvetica','B',14);$pdf->SetTextColor(0,212,170);$pdf->SetXY(14,$y_totals+8);$pdf->Cell(40,7,$grand_qty,0,0);
    if($remarks){$pdf->SetFont('helvetica','B',7);$pdf->SetTextColor(90,122,154);$pdf->SetXY(14,$y_totals+18);$pdf->Cell(80,4,'REMARKS',0,1);$pdf->SetFont('helvetica','',8);$pdf->SetTextColor(208,228,247);$pdf->SetXY(14,$y_totals+22);$pdf->MultiCell(80,5,$remarks,0,'L');}
    $tx=110;$ty=$y_totals;$pdf->SetFillColor(22,38,58);$pdf->Rect($tx,$ty,90,65,'FD');$disc_calc=$gross_amount*$disc_pct/100;
    $total_rows=array(array('Gross Amount',number_format($gross_amount,2),false),array('Print Charges',number_format($grand_print,2),false),array('Discount ('.number_format($disc_pct,2).'%)','- '.number_format($disc_calc,2),false),array('Discount Flat','- '.number_format($disc_flat,2),false),array('Extra Charges',number_format($extra_charges,2),false),array('NET AMOUNT',number_format($net_amount,2),true));
    $row_ty=$ty+3;foreach($total_rows as $tr_item){$label=$tr_item[0];$val=$tr_item[1];$is_grand=$tr_item[2];if($is_grand){$pdf->SetFillColor(0,74,136);$pdf->Rect($tx+2,$row_ty,86,10,'F');$pdf->SetFont('helvetica','B',11);$pdf->SetTextColor(0,212,170);}else{$pdf->SetFont('helvetica','',8);$pdf->SetTextColor(208,228,247);}$offset=$is_grand?2:1;$pdf->SetXY($tx+4,$row_ty+$offset);$pdf->Cell(50,5,$label,0,0,'L');$pdf->SetXY($tx+4,$row_ty+$offset);$pdf->Cell(82,5,'Rs. '.$val,0,0,'R');$row_ty+=($is_grand?11:8);}
    if($payment_modes){$pdf->SetFont('helvetica','B',7);$pdf->SetTextColor(90,122,154);$pdf->SetXY($tx+4,$row_ty+1);$pdf->Cell(82,4,'PAYMENT MODES',0,1);$pdf->SetFont('helvetica','',8);$pdf->SetTextColor(208,228,247);foreach(explode(',',$payment_modes) as $pm){$pm=trim($pm);if($pm){$pdf->SetXY($tx+4,$pdf->GetY());$pdf->Cell(82,5,$pm,0,1,'L');}}}
    $pdf->SetY(max($pdf->GetY(),$y_totals+68));$pdf->SetFillColor(0,74,136);$pdf->SetDrawColor(0,170,255);$pdf->SetLineWidth(0.5);$pdf->Rect(10,$pdf->GetY(),190,12,'FD');$pdf->SetFont('helvetica','B',12);$pdf->SetTextColor(255,255,255);$pdf->SetXY(14,$pdf->GetY()+3);$pdf->Cell(100,6,'BALANCE DUE',0,0,'L');$pdf->Cell(86,6,'Rs. '.number_format($balance_due,2),0,0,'R');
    try{$pdf->Output($filepath,'F');}catch(Exception $e){$buffered=ob_get_clean();header('Content-Type: application/json');echo json_encode(array('success'=>false,'error'=>'PDF output error: '.$e->getMessage()));exit;}
...
:root {
    ...
    --accent:#00aaff;--accent2:#00d4aa;--text:#d0e4f7;--text-dim:#7a9ab8;--text-label:#5a7a9a;
...
body{font-family:'Barlow',sans-serif;background:#0f1720;min-height:100vh;padding:22px;color:var(--text);font-size:18px;}
.container{max-width:1900px;margin:0 auto;}
.topbar{background:var(--header-bg);border:1px solid var(--border);border-bottom:2px solid var(--accent);border-radius:6px 6px 0 0;padding:10px 26px;display:flex;align-items:center;justify-content:space-between;font-size:17px;color:var(--text-dim);font-family:'Share Tech Mono',monospace;}
.topbar .brand{color:var(--accent);font-size:19px;font-weight:bold;letter-spacing:1px;}
.topbar .info-pills{display:flex;gap:16px;align-items:center;}
.topbar .pill{background:var(--panel2);padding:4px 14px;border-radius:3px;border:1px solid var(--border);font-size:15px;}
.topbar .pill span{color:var(--accent2);}
.banner{padding:16px 26px;font-size:18px;font-weight:600;border:1px solid;margin-bottom:0;}
...
.header-panel{background:var(--panel);border:1px solid var(--border);border-top:none;padding:16px 24px;display:grid;grid-template-columns:repeat(9,1fr);gap:10px;align-items:end;}
.header-panel .shop-name-block{grid-column:span 2;}
.shop-name-block h1{font-size:30px;font-weight:700;color:var(--accent);letter-spacing:0.5px;line-height:1.1;}
.shop-name-block p{font-size:16px;color:var(--text-dim);margin-top:3px;}
.hfield{display:flex;flex-direction:column;gap:3px;}
.hfield label{font-size:15px;text-transform:uppercase;letter-spacing:0.5px;color:var(--text-label);font-weight:600;}
.hfield input,.hfield select{background:var(--input-bg);border:1px solid var(--border);border-radius:3px;color:var(--text);font-size:18px;font-family:'Share Tech Mono',monospace;padding:8px 12px;width:100%;transition:border-color 0.2s;}
.hfield input:focus,.hfield select:focus{outline:none;border-color:var(--accent);background:#0d1520;}
.hfield input[readonly]{color:var(--accent2);}
.invoice-badge{background:var(--highlight);border:1px solid var(--accent);border-radius:3px;padding:7px 12px;font-family:'Share Tech Mono',monospace;font-size:26px;color:#fff;font-weight:bold;text-align:center;}
.tabs{background:var(--header-bg);border:1px solid var(--border);border-top:none;display:flex;gap:2px;padding:5px 10px 0;}
.tab{padding:11px 32px;font-size:17px;font-weight:600;border-radius:4px 4px 0 0;cursor:pointer;border:1px solid transparent;border-bottom:none;color:var(--text-dim);background:transparent;letter-spacing:0.3px;}
...
.grid-table{width:100%;border-collapse:collapse;font-size:18px;min-width:960px;}
.grid-table thead tr{background:var(--header-bg);}
.grid-table thead th{padding:11px 13px;text-align:center;font-size:15px;font-weight:700;text-transform:uppercase;letter-spacing:0.4px;color:var(--text-dim);border-right:1px solid var(--border);border-bottom:2px solid var(--border-light);white-space:nowrap;}
...
.grid-table tbody td:first-child{width:38px;text-align:center;color:var(--text-dim);font-family:'Share Tech Mono',monospace;font-size:15px;}
.grid-table tbody td input,.grid-table tbody td select{width:100%;background:transparent;border:none;color:var(--text);font-size:18px;font-family:'Barlow',sans-serif;padding:5px 8px;}
...
.sr-pattern-sel,.sr-fabric-inp,.sr-color-sel{background:transparent;border:none;color:var(--text);font-size:17px;font-family:'Barlow',sans-serif;padding:3px 5px;width:100%;}
.sr-pattern-sel:focus,.sr-fabric-inp:focus,.sr-color-sel:focus{outline:none;background:var(--input-bg);border-radius:2px;}
.size-rate-panel table{width:100%;border-collapse:collapse;font-size:17px;}
.size-rate-panel th{background:var(--header-bg);color:var(--text-dim);font-size:15px;text-transform:uppercase;padding:4px 6px;border:1px solid var(--border);}
.size-rate-panel td{border:1px solid var(--border);padding:2px;}
.size-rate-panel input{background:transparent;border:none;color:var(--text);font-size:17px;font-family:'Share Tech Mono',monospace;padding:3px 5px;width:100%;}
.size-rate-panel input:focus{outline:none;background:var(--input-bg);border-radius:2px;}
.rm-size-btn{background:transparent;border:none;color:var(--btn-red);cursor:pointer;font-size:17px;padding:0 4px;}
.rm-size-btn:hover{color:#ff6060;}
.add-size-btn{background:transparent;border:1px dashed var(--border-light);color:var(--accent);font-size:16px;font-weight:600;padding:4px 10px;cursor:pointer;border-radius:0;margin:0;width:100%;text-align:center;display:block;}
...
.sfield label{font-size:15px;text-transform:uppercase;letter-spacing:0.5px;color:var(--text-label);font-weight:600;}
.sfield input{background:var(--input-bg);border:1px solid var(--border);border-radius:3px;color:var(--text);font-size:17px;font-family:'Share Tech Mono',monospace;padding:6px 10px;width:100%;}
...
.pc-footer-title{font-size:15px;font-weight:700;text-transform:uppercase;letter-spacing:0.5px;color:var(--text-label);display:flex;align-items:center;gap:6px;}
...
.pc-inp-label{font-size:15px;color:var(--text-label);white-space:nowrap;font-weight:600;text-transform:uppercase;}
.pc-inp{background:transparent;border:none;border-bottom:1px solid var(--border-light);color:var(--accent2);font-family:'Share Tech Mono',monospace;font-size:17px;text-align:right;padding:2px 4px;width:100%;}
...
.pc-qty-note{font-size:16px;color:var(--text-dim);font-family:'Share Tech Mono',monospace;}
.pc-total-val{font-family:'Share Tech Mono',monospace;font-size:22px;font-weight:bold;color:#f0a020;}
...
.total-row label{font-size:16px;font-weight:600;color:var(--text-dim);text-transform:uppercase;letter-spacing:0.3px;}
.total-row .val{font-family:'Share Tech Mono',monospace;font-size:19px;color:var(--text);}
.total-row.grand{border-bottom:none;margin-top:5px;}
.total-row.grand label{color:var(--accent2);font-size:18px;}
.total-row.grand .val{font-size:30px;color:var(--accent2);font-weight:bold;}
.total-input-wrap{display:flex;align-items:center;gap:8px;}
.total-inline-input{background:var(--input-bg);border:1px solid var(--border);border-radius:3px;color:var(--accent2);font-family:'Share Tech Mono',monospace;font-size:18px;text-align:right;padding:7px 12px;width:125px;}
...
.paymodes-title{font-size:15px;font-weight:700;text-transform:uppercase;letter-spacing:0.6px;color:var(--text-label);border-bottom:1px solid var(--border);padding-bottom:6px;margin-bottom:2px;}
.paymode-entry{display:grid;grid-template-columns:1fr 120px 32px;align-items:center;gap:7px;}
.paymode-select{background:var(--input-bg);border:1px solid var(--border);border-radius:4px;color:var(--text);font-size:17px;padding:9px 13px;width:100%;}
.paymode-select:focus{outline:none;border-color:var(--accent);}
.paymode-amount{background:var(--input-bg);border:1px solid var(--border);border-radius:4px;color:var(--accent2);font-size:18px;font-family:'Share Tech Mono',monospace;text-align:right;padding:9px 13px;width:100%;}
.paymode-amount:focus{outline:none;border-color:var(--accent);}
.paymode-amount::placeholder{color:var(--text-label);}
.paymode-remove{background:transparent;border:1px solid var(--border);border-radius:4px;color:var(--btn-red);font-size:17px;width:34px;height:34px;cursor:pointer;display:flex;align-items:center;justify-content:center;}
.paymode-remove:hover{background:rgba(224,64,64,0.15);border-color:var(--btn-red);}
.paymode-add-btn{background:transparent;border:1px dashed var(--border-light);border-radius:4px;color:var(--accent);font-size:16px;font-weight:600;padding:6px 12px;cursor:pointer;width:100%;text-align:center;}
...
.balance-row label{font-size:16px;font-weight:700;color:var(--accent2);text-transform:uppercase;}
.balance-val{font-family:'Share Tech Mono',monospace;font-size:27px;font-weight:bold;color:var(--accent2);}
...
.btn{padding:13px 34px;border:none;border-radius:4px;font-size:18px;font-weight:700;letter-spacing:0.5px;cursor:pointer;transition:filter 0.15s,transform 0.1s;font-family:'Barlow',sans-serif;}
.btn:hover{filter:brightness(1.15);}
.btn:active{transform:scale(0.97);}
.btn-confirm{background:var(--btn-green);color:#000;}
.btn-print{background:var(--btn-blue);color:#fff;}
.btn-clear{background:var(--btn-gray);color:#ccc;}
...
.wa-float-tooltip{position:fixed;bottom:40px;right:108px;background:#075E54;color:#fff;font-size:15px;font-weight:600;padding:6px 14px;border-radius:20px;white-space:nowrap;opacity:0;pointer-events:none;transition:opacity 0.2s;z-index:9997;}
...
.modal-icon{width:34px;height:34px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:17px;flex-shrink:0;}
...
.modal-title{font-size:18px;font-weight:700;text-transform:uppercase;color:var(--text);font-family:'Share Tech Mono',monospace;}
.modal-body{padding:22px 24px;font-size:17px;color:var(--text-dim);line-height:1.6;}
.modal-footer{padding:12px 20px 18px;display:flex;justify-content:flex-end;gap:8px;}
.modal-btn{padding:11px 30px;border:none;border-radius:4px;font-size:17px;font-weight:700;cursor:pointer;font-family:'Barlow',sans-serif;}
.modal-btn-ok{background:var(--accent);color:#000;}
.modal-btn-yes{background:#f0a020;color:#000;}
.modal-btn-cancel{background:var(--btn-gray);color:#ccc;}
...
.wa-phone-hint-flag{font-size:16px;}
.wa-phone-hint code{
...
.wa-btn-cancel:hover{background:rgba(255,255,255,0.08);color:#8aaabb;}
.wa-btn-open{
...
<div class="container">
    ...
    <div class="header-panel">
        ...
        <div class="hfield"><label>Entry No.</label><div class="invoice-badge" id="invoiceDisplay">—</div></div>
        <div class="hfield"><label>Invoice No.</label><input type="text" id="invoice_no" placeholder="Auto" readonly></div>
        <div class="hfield"><label>Customer Name</label><input type="text" id="customer_name" placeholder="Enter name"></div>
        <div class="hfield"><label>Order Date</label><input type="date" id="order_date"></div>
        <div class="hfield"><label>Delivery Date</label><input type="date" id="delivery_date"></div>
        <div class="hfield"><label>Advance Date</label><input type="date" id="advance_date"></div>
        <div class="hfield"><label>Type</label>
            <select id="bill_type"><option>Regular Bill</option><option>Advance Order</option><option>Bulk Order</option></select>
        </div>
        <div class="hfield"><label>Status</label>
            <select id="bill_status"><option>Pending</option><option>Ready</option><option>Delivered</option></select>
        </div>
    </div>
...
// ─── WhatsApp Phone Number Modal (NEW — replaces browser prompt) ─────────────
...
const productData={
    ...
    shoes:{sizes:["6","7","8","9","10","11"],patterns:[]},
...
// ─── Table Builder ───────────────────────────────────────────────────────────
function buildTable(){const tbody=document.getElementById('tableBody');tbody.innerHTML='';for(let i=1;i<=NUM_ROWS;i++)addRow(i);}
function addRow(num){
    ...
    tr.innerHTML=`
        ...
        <td style="min-width:640px;padding:0;">
            <div class="size-rate-panel" id="srp-${idx}">
                <table><thead><tr>
                    <th style="width:56px;">Size</th>
                    <th style="width:120px;">Pattern</th><th style="width:110px;">Fabric</th><th style="width:120px;">Color</th>
                    <th style="width:78px;">MRP ₹</th><th style="width:48px;">Dis%</th><th style="width:82px;">Net Rate</th>
                    <th style="width:54px;">Qty</th><th style="width:85px;">Amount</th><th style="width:24px;"></th>
                </tr></thead>
                <tbody class="sr-body"></tbody></table>
                <button class="add-size-btn" onclick="addNextSizeRow(${idx})">+ Size</button>
            </div>
        </td>
...
function addSizeRateRow(rowIdx,sizeValue='',rateValue='',doFocus=true){
    ...
    tr.innerHTML=`
        <td><input type="text" class="sr-size" placeholder="Size" value="${sizeValue}" onkeydown="srAutoJump(event,this,'size')" style="width:54px;background:transparent;border:none;color:var(--text);font-size:17px;font-family:'Share Tech Mono',monospace;padding:4px 6px;"></td>
        ...
        <td><input type="number" class="sr-mrp" placeholder="0" value="${rateValue}" min="0" onkeydown="srAutoJump(event,this,'mrp')" oninput="calcSizeRow(this)" style="width:76px;background:transparent;border:none;color:var(--text);font-size:17px;font-family:'Share Tech Mono',monospace;padding:4px 6px;"></td>
        <td><input type="number" class="sr-dis" placeholder="0" value="0" min="0" max="100" onkeydown="srAutoJump(event,this,'dis')" oninput="calcSizeRow(this)" style="width:46px;background:transparent;border:none;color:var(--text);font-size:17px;font-family:'Share Tech Mono',monospace;padding:4px 6px;"></td>
        <td><input type="number" class="sr-netrate" placeholder="0.00" readonly style="width:80px;background:transparent;border:none;color:var(--accent2);font-size:17px;font-family:'Share Tech Mono',monospace;padding:4px 6px;"></td>
        <td><input type="number" class="sr-qty" placeholder="0" min="0" onkeydown="srAutoJump(event,this,'qty')" oninput="calcSizeRow(this)" style="width:52px;background:transparent;border:none;color:var(--text);font-size:17px;font-family:'Share Tech Mono',monospace;padding:4px 6px;"></td>
        <td><input type="number" class="sr-amount" placeholder="0.00" readonly style="width:83px;background:transparent;border:none;color:var(--accent2);font-size:15px;font-family:'Share Tech Mono',monospace;padding:3px 5px;font-weight:bold;"></td>
...
function srAutoJump(e,inp,fieldType){
    if(e.key==='Tab'||e.key==='Enter'){
        e.preventDefault();
        const tr=inp.closest('tr');
        if(fieldType==='size')tr.querySelector('.sr-mrp')?.focus();
        else if(fieldType==='mrp')tr.querySelector('.sr-dis')?.focus();
        else if(fieldType==='dis')tr.querySelector('.sr-qty')?.focus();
...
function deleteRow(){
    ...
    if(active&&document.querySelectorAll('#tableBody tr').length>1){active.remove();updateTotals();}
}
    ...
    items.forEach(item=>{
        ...
        sizes.forEach((sz,si)=>{
            ...
            itemRows+=`<tr>
                ...
                <td>${qty}</td>
                <td>₹${Number(sz.amount||0).toFixed(2)}</td>
            </tr>`;
        });
...
table.items tbody td{padding:9px 8px;font-size:20px;font-weight:900;text-align:center;border:1px solid #888;vertical-align:top;}
table.items tfoot tr{background:#003366;color:#fff;}
table.items tfoot td{padding:10px 8px;font-weight:900;font-size:22px;border:1px solid #003366;text-align:center;}
...
.bill-footer{margin-top:14px;border-top:2px solid #003366;padding-top:8px;display:flex;justify-content:space-between;align-items:flex-end;}
.footer-note{font-size:19px;font-weight:900;color:#000;}
.sign-box{text-align:center;font-size:19px;font-weight:900;color:#000;}
.sign-line{border-top:1px solid #000;margin-top:28px;width:130px;}
...
<div class="info-strip">
  ...
  <div class="info-cell">
    ...
    <div class="val">${billType}</div>
  </div>
  <div class="info-cell">
    <div class="lbl">Status</div>
    <div class="val">${billStatus}</div>
  </div>
</div>
<table class="items">
  <thead>
  ...
  <tfoot>
    <tr>
      <td colspan="9" style="text-align:right;padding-right:12px;">TOTAL QTY &amp; GROSS</td>
      <td>${grandQty}</td>
      <td>${gross}</td>
    </tr>
  </tfoot>
...
<div class="bottom-section">
  ...
  <div>
    ...
    <div class="balance-line">
      ...
      <span class="b-val">${balance}</span>
    </div>
  </div>
</div>
${remarks?`<div class="remarks-bar"><strong>📝 Remarks:</strong> ${remarks}</div>`:''}
<div class="bill-footer">
  <div class="footer-note">Thank you for your order!<br>Goods once sold will not be taken back.</div>
  <div class="sign-box">
...
.bill-footer{margin-top:16px;border-top:2px solid #003366;padding-top:8px;display:flex;justify-content:space-between;align-items:flex-end;}
...
<div class="bill-header">
  ...
  <div style="text-align:right;">
    <div class="mc-badge">⚙ MASTER COPY</div>
    <div style="font-size:18px;color:#000;margin-top:4px;">Invoice: <strong>${invNo}</strong></div>
  </div>
</div>
<div class="info-strip">
...
${remarks?`<div class="remarks-bar"><strong>📝 Remarks:</strong> ${remarks}</div>`:''}
<div class="bill-footer">
  <div class="footer-note">Sarthi Sports Wear &nbsp;|&nbsp; Ph: 0762-0425141 / 9422107750<br>This is a production/work order copy — not a payment receipt.</div>
  <div class="sign-box">
    <div class="sign-line"></div>
    Authorised Signatory
  </div>
...
async function sendWhatsApp(){
    ...
    // ── CHANGED: replaced browser prompt() with stylish modal ──
    ...
    try{
        ...
        if(contentType.includes('application/octet-stream')||contentType.includes('application/pdf')){
            ...
            // ── CHANGED: opens WhatsApp inbox only — no pre-filled text ──
